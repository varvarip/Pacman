...
public class Creature implements gr.auth.ee.dsproject.pacman.AbstractCreature
{
  ...
  public int calculateNextPacmanPosition (Room[][] Maze, int[] currPosition)
  {
    ...
    ArrayList<double[]> arrayEval = createSubTreePacman(1, parentNode,Maze,currPosition);
    
    double[] rock = new double[arrayEval.size()];    
    for (int i=0; i < arrayEval.size(); i++) {
    	double[] punk = arrayEval.get(i);    	
    	double min = punk[0];
    	for(int j=0; j<punk.length; j++) {
    		if (min>punk[i]) {
    			min = punk[i];
    		}
    	}
  ...
  ArrayList<double[]> createSubTreePacman (int depth, Node parent, Room[][] Maze, int[] currPacmanPosition)
  {
	  ////////// traffic lights ////////////////
	  int d,g,l;	
	...
	if (validMoves.isEmpty()) {
		...
		ArrayList<double[]> ghostEval = new ArrayList<double[]>();
		...
		for(int i =0; i<validMoves.size();i++) {
			...
			ghostEval.add(i,createSubTreeGhosts(depth+1,istChildNode,roomCopy1,istChildNode.getGhostPos()));
  ...
  double[] createSubTreeGhosts (int depth, Node parent, Room[][] Maze, int[][] currGhostsPosition)
  {
	  ArrayList<int[][]> ghostMovesArray = PacmanUtilities.allGhostMoves(Maze, currGhostsPosition);
	  double[] iEval = new double[ghostMovesArray.size()];
